...
import { StyleSheet, Text ,Image, View, Dimensions, FlatList, Animated, findNodeHandle } from 'react-native';
...
const Tabs = ({data, scrollX, onItemPress})=>{
  ...
  <View  style={{position:"absolute", top: 30, width}}>
    <View ref={containerRef} style={{justifyContent:"space-evenly", flex: 1, flexDirection:"row"}}>
      {data.map((item, index)=>{
        return <Tab key={item.key} item={item} ref={item.ref} onItemPress={()=> onItemPress(index)}></Tab>
      })}
    </View>
    {measures.length > 0 && <Indicator measures={measures} scrollX={scrollX}></Indicator>}
  </View>
  )
}
